...
public class StoreAcceptListPagingControl implements Controller {
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<Store> pageList = service.getacceptstoreList(cri);
		
		req.setAttribute("list", pageList);
		
		List<Store> totalList = service.acceptstoreList();
		int total = totalList.size();
		req.setAttribute("pageInfo", new Page(cri, total));
		
		Utils.forward(req, resp, "store/StoreAcceptList.tiles");
